							...
							<table class="table">
								...
								<tbody>
									<tr v-for="(value, field) in selectedItem">
										...
										<td v-bind:data-craftercms-model-id="selectedItem.itemUrl"   v-bind:data-craftercms-model-path="selectedItem.itemUrl"  v-bind:data-craftercms-field-id="{{field}}" v-else>{{ value }}</td>
									</tr>
								</tbody>
							</table>
						</div>
